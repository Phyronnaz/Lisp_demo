...
// TODO
std::vector<std::string> List_to_string_vector(const Object& o){
    std::vector<std::string> v;
    auto to_convert = o;
    while(!null(to_convert)){
        v.push_back(Object_to_string(car(to_convert)));
...
// TODO
bool is_list_of_string(const Object& o){
    if (null(o)){
        return true;
    }
    else if(listp(o) && stringp(car(o))){
        return is_list_of_string(cdr(o));
    }
    else{
        return false;
    }
